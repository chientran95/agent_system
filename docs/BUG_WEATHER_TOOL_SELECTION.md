...
## Update: input-required redesigned as a structured-output field, not a separate tool

The original design had a standalone `ask_user` tool the main `agent` node's model could choose to call, alongside the real weather tools - "decide whether to invoke a side-channel tool" turned out to be exactly the kind of behavior that's been unreliable throughout this project (mirroring `research_agent`'s `call_code_agent` being skipped even when relevant).

Redesigned so the question is a **required-or-null field on a call the model already has to make**, rather than a discrete tool-invocation decision: `ToolSelection` now has an optional `clarifying_question: str | None` field alongside `tool_names`. `select_tools_node` checks it first - if set, it calls `interrupt()` immediately (before `agent_node` is ever reached), folds the answer back into the conversation, and retries selection (bounded by `_MAX_CLARIFICATION_ROUNDS = 2` as a safety cap against looping forever, not an expected number of rounds). The standalone `ask_user` tool is kept as a secondary fallback for `agent_node` (e.g. if a tool error only reveals missing information *after* an attempt), but the primary path no longer depends on the model separately deciding to invoke it.

Verified live end-to-end against the real NVIDIA-hosted model: an underspecified query ("What is the weather like today?") correctly reached `input-required` with a real question, resumed cleanly, and completed with a correct final answer - even gracefully surviving one transient `geocoding` blank-error retry along the way (the agent's own existing "retry with different params" guidance handled it). A well-specified query ("current weather in Paris, France") proceeded directly with no spurious clarification request.

Along the way, found and fixed a real, separate bug this surfaced: `_astream_graph` assumed `message.content` was always a string, but MCP tool results come back as a list of content blocks (`[{"type": "text", "text": "..."}]`). Yielding that list as-is broke any caller expecting a string. Added a `_content_to_text()` helper that extracts each block's actual text instead of yielding the raw list or its Python repr.

**Same design extended to `research_agent`**, with a different implementation mechanism since its graph isn't hand-built like `weather_agent`'s - `research_agent.py` uses `deepagents.create_deep_agent()`, which owns its own internal graph construction. Instead of a custom `StateGraph` node, `ClarifyTopicMiddleware(AgentMiddleware)` hooks into `abefore_agent` (from `langchain.agents.middleware.types`), passed via `create_deep_agent(middleware=[...])`. This runs once, before any research/model work begins, as a real node in the *same* compiled graph (confirmed via `agent.client.get_graph().nodes.keys()` showing `ClarifyTopicMiddleware.before_agent`) - so it shares the same checkpointer and thread_id as everything else, no separate wrapper graph or phase-tracking needed. Same `clarifying_question`-field-on-a-structured-call design, same `interrupt()`-and-fold-answer-back-in loop, same `_content_to_text()` defensive fix applied preemptively (not yet observed failing there, but the same risk exists for any future list-returning tool).

Verified live: a deliberately vague topic ("Research that thing and write about it.") correctly reached `input-required` ("What specific thing are you referring to? ..."), and resuming with a real topic correctly folded the answer back in and proceeded. One honest caveat, unrelated to this change: after resuming, the model sometimes answers directly from its own knowledge instead of following the full `web_search` → `content_writer` procedure - this is the same pre-existing deep-agent procedure-skipping unreliability already documented elsewhere in this project (e.g. `call_code_agent` being skipped), not a regression from the clarification middleware, which itself worked correctly and independently of it.
