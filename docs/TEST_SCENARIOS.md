...
- **`research_agent`'s `mistral-small3.2:24b` (via Ollama) sometimes invents tool-call kwargs instead of using the declared schema** (scenario 2.2) — e.g. calling `call_code_agent` with `{'description': ...}` instead of its actual `request` argument, or deepagents' built-in `task` tool with `{'task_description': ...}` instead of `description`. Seen against two independently-schema'd tools (one ours, one deepagents' own), so it isn't a docstring-wording issue on our side. A `ChatNVIDIA` migration (mirroring `weather_agent`'s fix) was tried and reverted — it surfaced a second, NVIDIA-endpoint-specific issue (`400: Assistant message must have either content or tool_calls, but not both`, since the model narrates before calling a tool and that message re-enters history verbatim) that would need its own fix. For now this is treated the same as the LiteLLM/Ollama flakiness above: retry, no root-cause fix in place.
