...
- **`weather_agent` fails to select a tool from its 17-tool MCP toolset** (scenarios 1.4/1.5). A real context-window truncation bug is fixed and verified (`ChatOllama` now sets `num_ctx=65536`); a second, deeper problem remains unresolved where the model still fails to call any tool even with full, untruncated visibility of all 17 schemas. Full write-up: [BUG_WEATHER_TOOL_SELECTION.md](BUG_WEATHER_TOOL_SELECTION.md).
